...
- Keep `docs/` in sync with every change:
	- New behavior → append a new `US-###` / `REQ-###` / `OUT-###` to `docs/PRD.md`; update `docs/ARCHITECTURE.md` if modules, data flow, schema, or external contracts change. (`MET-###` is retired — measurable targets live inside `REQ`s.)
	- Changed behavior → propose the edit and **ask for explicit user confirmation** before mutating an existing entry.
	- IDs are append-only and never reused. Removed items are **moved** to the **Removed / superseded** section at the bottom of `docs/PRD.md` (not left tombstoned in their original list), with a note in the relevant spec.
